{% macro recipe_normalize(value, apply_aliases=true) %}
    ...
    {% set text = "trim(regexp_replace(regexp_replace(" ~ text ~ ", '\\b(about|added|as|coarse|coarsely|divided|diced|finely|fresh|freshly|for|good|ground|large|medium|melted|more|new|plus|quality|room|small|such|temperature|thinly|torn|whole|chopped|sliced|packed|or|of|at|least|kosher|sturdy|dry|natural|unsalted|crushed|optional|storebought|homemade|needed|serving|serve|serves|garnish|to|taste|only|lightly|roughly|removed|drained|rinsed|halved|lengthwise|crosswise|beaten|grated|minced|peeled|seeded|cored|cut|cubes|pieces|finishing|separated|tender|parts|white|pale|according|directions)\\b', ' ', 'g'), '[[:space:]]+', ' ', 'g'))" %}
